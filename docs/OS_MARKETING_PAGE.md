...
Status: implemented at `/os` as a public concept page. Cycle 1 of the free core is public at https://github.com/1ststepai/1ststep-os and runs locally (`npm install && npm test && npm run build && npm start`, then http://localhost:3000/os) as idea → profile → Project OS markdown ZIP. That local vertical is not a hosted product, full agent OS, or control plane. The current opening positions the intended system as an **AI Engineering Operating System**: context → authority → isolated work → evidence → release decision. Broader product, business, and discovery concepts below remain visibly labeled as future or illustrative. Run commands for Cycle 1 belong on `/tools/1ststep-os/`. This document covers presentation only; application architecture remains in the OS repo.
...
- Current: public concept page, public GitHub free core, and Cycle 1 local ZIP demo. In development: onboarding, adapters, routing, capability workflows, and a hosted product. Designed for/coming: media factory, discovery intelligence, resumability, opportunity engine, and command center. None of the conceptual scores represent a real project. The marketing-page prompt is illustrative and does not generate a ZIP.
