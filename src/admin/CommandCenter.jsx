...
function Records({ items, type, coverage }) {
  ...
  const secondary = (item) => {
    if (type === "Handoffs") return `${item.from} → ${item.to} · Role: ${item.recipientRole || "UNKNOWN"} · Sent ${when(item.sentAt)} · Delivered ${when(item.deliveredAt)} · Acknowledged ${when(item.acknowledgedAt)} · Finding: ${item.findingId || "UNKNOWN"} · Truth: ${item.truthState || "UNKNOWN"}`;
