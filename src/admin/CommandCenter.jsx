...
function AuditCapacity({ value }) {
  return <Card><div className="cc-card-head"><div><p className="cc-eyebrow">Audit router / read-only</p><h3>Audit Capacity</h3></div><span className="cc-muted">{value ? `Observed ${when(value.observedAt)}` : "Source unknown"}</span></div>
    {value ? <><div className="cc-list">{value.providers.map((provider) => <div className="cc-row" key={provider.id}><div className="cc-row-main"><strong>{provider.label}</strong><small>{provider.model || "Current model unknown"}</small></div><Badge value={provider.status} /></div>)}</div>
      <div className="cc-metrics" aria-label="Audit queue"><div><strong>{value.auditQueue ?? "UNKNOWN"}</strong><span>Audit queue</span></div><div><strong>{value.activeJobs ?? "UNKNOWN"}</strong><span>Active jobs</span></div><div><strong>{value.autoRoutable ?? "UNKNOWN"}</strong><span>Auto-routable</span></div><div><strong>{value.claudeEscalation ?? "UNKNOWN"}</strong><span>Claude-only pending</span></div></div>
      <div className="cc-list" aria-label="Latest audit jobs">{value.jobs?.length ? value.jobs.map((job) => <div className="cc-row" key={job.id}><div className="cc-row-main"><strong>{job.findingId || job.id} · {job.project || "Project unknown"}</strong><small>{job.provider || "Provider unselected"} · {job.model || "Model unknown"} · Cost {job.costUsd === 0 ? "$0.00" : "UNKNOWN"} · {when(job.updatedAt)}</small></div><Badge value={job.verdict || job.status} /></div>) : <p className="cc-freshness">No durable AuditJobs reported.</p>}</div></> : <Empty label="Audit provider status unknown" coverage={false} />}
