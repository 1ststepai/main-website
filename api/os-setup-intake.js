...
export default async function handler(req, res) {
  ...
  const limit = checkRateLimit(req, 'os-setup-intake');
  ...
  try {
    ...
    return reply(res, status, { ok: false, code: error.code || (status === 400 ? 'invalid_request' : 'intake_unavailable'), message: status >= 500 ? 'We could not save your request. Please try again.' : error.message });
  }
}
