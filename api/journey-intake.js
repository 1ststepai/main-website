...
import { enforceJourneyRateLimit, saveJourneyRequest } from "../lib/journey/intakeStore.js";
...
export default async function handler(req, res) {
  ...
  const limit = checkRateLimit(req, "journey-intake");
  ...
  try {
    ...
    if (Buffer.byteLength(raw, "utf8") > 8192) return reply(res, 413, { ok: false, code: "payload_too_large" });
    await enforceJourneyRateLimit(req);
    const result = await saveJourneyRequest(JSON.parse(raw));
    ...
    return reply(res, status, {
      ok: false,
      code: error.code || (status === 400 ? "invalid_request" : "intake_unavailable"),
      message: status >= 500 ? "The request service is unavailable. Your answers remain here; please try later." : error.message,
    });
  }
}
