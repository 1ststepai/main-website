...
async function scanWebsite(target, sourceType = target.sourceType || 'website', maxBytes = MAX_HTML_BYTES) {
  let url = new URL(target.url);
  let response;
  for (let hop = 0; hop < 3; hop++) {
    response = await requestPinned(url, { maxBytes });
...
async function scanMobileApp(target) {
  ...
  return scanWebsite(target, 'mobile_app', 1536 * 1024);
}
