...
export function parseTarget(input) {
  ...
  if (url.hostname === 'github.com' || url.hostname === 'www.github.com') {
    ...
    return { kind: 'github', url: `https://github.com/${parts[0]}/${repo}`, owner: parts[0], repo };
  }
...
async function scanWebsite(target) {
  ...
  return { kind: 'website', target: target.url, inspectedUrl: url.toString(), checkedAt: new Date().toISOString(), source: 'Public HTML response', checks, nextStep, note: 'This first look inspects one public HTML response. JavaScript-rendered content, accessibility behavior, security, performance, analytics, and private systems remain unverified.' };
}
