...
test('private Blob rate limiting retains only a connection hash and rejects the thirteenth request', async (t) => {
  ...
  const acceptedCount = client.records.size;
  await assert.rejects(() => enforceProtectedLeadRateLimit(req, 'os-roast', client), { code: 'rate_limited', statusCode: 429 });
  await assert.rejects(() => enforceProtectedLeadRateLimit(req, 'os-roast', client), { code: 'rate_limited', statusCode: 429 });
  assert.equal(client.records.size, acceptedCount);
  assert.equal(acceptedCount, 12);
  assert.equal([...client.records.keys()].some((pathname) => pathname.includes('203.0.113.88')), false);
});

test('parallel Blob intake attempts cannot create more than twelve rate-limit slots', async (t) => {
  const prior = process.env.BLOB_READ_WRITE_TOKEN;
  t.after(() => { if (prior === undefined) delete process.env.BLOB_READ_WRITE_TOKEN; else process.env.BLOB_READ_WRITE_TOKEN = prior; });
  process.env.BLOB_READ_WRITE_TOKEN = 'vercel_blob_test';
  const client = privateBlobClient();
  const req = { headers: { 'x-forwarded-for': '203.0.113.89' } };
  const results = await Promise.allSettled(Array.from({ length: 30 }, () => enforceProtectedLeadRateLimit(req, 'os-roast', client)));
  assert.equal(results.filter((result) => result.status === 'fulfilled').length, 12);
  assert.equal(client.records.size, 12);
  assert.ok(results.filter((result) => result.status === 'rejected').every((result) => result.reason.code === 'rate_limited'));
});
