...
const INTENTS = ["", "ai-operations-audit"];
...
export function normalizeJourneyRequest(input) {
  ...
  attribution.landing_path = typeof source.landing_path === "string" && source.landing_path.startsWith("/")
    ...
    : "";
  const intent = boundedText(input.intent ?? "", 40, false);
  if (!INTENTS.includes(intent)) throw failure("invalid_request", 400, "Please check your request.");
  return { request_id: requestId.toLowerCase(), email, intent, answers, attribution, consent: true };
}
...
export { command as protectedIntakeCommand };

export async function enforceJourneyRateLimit(req, namespace = 'journey') {
...
export async function listJourneyRequests(cursor = "0") {
  ...
  const requests = values.filter((value) => typeof value === "string").map((value) => {
    const { request_id, email, intent = "", answers, attribution, created_at } = decryptProtectedJson(value, "journey-intake");
    return { request_id, email, intent, answers, attribution, created_at };
  });
