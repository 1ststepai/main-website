    ...
    <div style="font-family:Arial,sans-serif;line-height:1.5;color:#111827">
      ...
      <table cellpadding="8" cellspacing="0" style="border-collapse:collapse;width:100%;max-width:760px">
        ${rows.map(([label, value]) => `
          <tr>
            <td style="border:1px solid #e5e7eb;font-weight:700;vertical-align:top;width:180px">${escapeHtml(label)}</td>
            <td style="border:1px solid #e5e7eb;vertical-align:top">${escapeHtml(value)}</td>
          </tr>
        `).join("")}
      </table>
  ...
  try {
    const response = await fetchWithTimeout("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `New App Idea Checker lead: ${lead.score}/100 - ${subjectName(lead.name)}`,
        text: emailText(lead),
        html: emailHtml(lead)
      })
    }, 8000);
