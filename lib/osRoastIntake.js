...
export function normalizeRoastRequest(input) {
  ...
  return { request_id: input.request_id.toLowerCase(), email, target: publicLink, kind: target.kind, consent: true, marketing_opt_in: input.marketing_opt_in === true, marketing_consent_version: input.marketing_opt_in === true ? MARKETING_CONSENT_VERSION : null };
}
...
export async function listRoastRequests(cursor = '0') {
  ...
  return { cursor: String(result[0]), requests: values.filter((value) => typeof value === 'string').map((value) => {
    const { request_id, email, target, kind, created_at, marketing_opt_in, marketing_consent_version } = decryptProtectedJson(value, 'os-roast-intake');
    return { request_id, email, target, kind, created_at, marketing_opt_in: marketing_opt_in === true, marketing_consent_version: marketing_opt_in === true ? marketing_consent_version || null : null };
  }) };
}
