...
export async function getSavedRoastScanTarget(requestId, input, sourceType) {
  if (typeof requestId !== 'string' || !/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(requestId)) return null;
  const target = parseTarget(input);
  const normalizedSource = normalizeSourceType(sourceType, target);
  const savedTarget = target.kind === 'github' || normalizedSource === 'mobile_app' ? target.url : `${new URL(target.url).origin}/`;
  const encrypted = await getProtectedLead(`${PREFIX}${requestId.toLowerCase()}`);
  if (!encrypted) return null;
  const record = decryptProtectedJson(String(encrypted), 'os-roast-intake');
  return record.consent === true && record.request_id === requestId.toLowerCase() && record.source_type === normalizedSource && record.target === savedTarget ? record.target : null;
}
