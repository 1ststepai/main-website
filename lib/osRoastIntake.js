...
export function normalizeRoastRequest(input) {
  ...
  return { request_id: input.request_id.toLowerCase(), email, target: publicLink, kind: target.kind, attribution: normalizeLeadAttribution(input.attribution), consent: true, marketing_opt_in: input.marketing_opt_in === true, marketing_consent_version: input.marketing_opt_in === true ? MARKETING_CONSENT_VERSION : null };
}
...
export async function listRoastRequests(cursor = '0') {
  ...
  return { cursor: result.cursor, requests: result.values.map((value) => {
    const { request_id, email, target, kind, attribution, created_at, marketing_opt_in, marketing_consent_version } = decryptProtectedJson(value, 'os-roast-intake');
    return { request_id, email, target, kind, attribution: normalizeLeadAttribution(attribution), created_at, marketing_opt_in: marketing_opt_in === true, marketing_consent_version: marketing_opt_in === true ? marketing_consent_version || null : null };
  }) };
}
