...
export async function enforceProtectedLeadRateLimit(req, namespace, blobClient = defaultBlobClient) {
  ...
  const result = await blobClient.list({ prefix, limit: 12 });
  ...
  for (let slot = 0; slot < 12; slot += 1) {
    try {
      await blobClient.put(`${prefix}${slot}.txt`, '1', {
        access: 'private',
        addRandomSuffix: false,
        allowOverwrite: false,
  ...
  throw Object.assign(new Error('Please wait before sending another request.'), { code: 'rate_limited', statusCode: 429 });
}
