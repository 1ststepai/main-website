...
function shipModeProjection(value) {
  if (value == null) return null;
  if (value.schemaVersion !== 1 || value.project !== "public-os" || !Array.isArray(value.currentWorkItems)) throw failure("invalid_ship_health");
  const health = ["ACTIVE", "WORKING", "WAITING_EXPLAINED", "BLOCKED", "STALE", "IDLE_FAULT", "CLOSED", "UNKNOWN"];
  const items = records(value.currentWorkItems, 100, (item) => ({
    id: opaque(item.id), state: opaque(item.state, 40), ownerRole: opaque(item.ownerRole, 80),
    health: health.includes(item.health) ? item.health : "UNKNOWN", eligible: item.eligible === true,
    lastHeartbeatAt: optionalTimestamp(item.lastHeartbeatAt), lastActivityAt: optionalTimestamp(item.lastActivityAt),
    waitingReason: optionalOpaque(item.waitingReason, 80),
    timeInStateMs: Number.isSafeInteger(item.timeInStateMs) && item.timeInStateMs >= 0 ? item.timeInStateMs : null,
  }));
  if (new Set(items.map((item) => item.id)).size !== items.length) throw failure("duplicate_ship_item");
  const eligibleUnowned = items.filter((item) => item.eligible && ["IDLE_FAULT", "STALE"].includes(item.health)).length;
  const staleAgents = items.filter((item) => item.health === "STALE").length;
  if (value.eligibleUnowned !== eligibleUnowned || value.staleAgents !== staleAgents ||
    (value.nextEligibleItem != null && !items.some((item) => item.id === value.nextEligibleItem && item.eligible))) throw failure("ship_health_mismatch");
  return { observedAt: timestamp(value.observedAt), project: "public-os", status: health.includes(value.status) ? value.status : "UNKNOWN",
    currentWorkItems: items, eligibleUnowned, staleAgents,
    nextEligibleItem: optionalOpaque(value.nextEligibleItem) };
}
...
export function normalizeCommandCenterSource(input, now = Date.now()) {
  ...
  return {
    ...
    shipMode: source === "public-ecosystem" ? shipModeProjection(input.shipMode) : null,
    agents,
...
export function summarizeCommandCenter(sources, now = Date.now()) {
  ...
  return {
    ...
    shipMode: (() => {
      const projection = live.find((item) => item.source === "public-ecosystem")?.data.shipMode;
      return projection && now - Date.parse(projection.observedAt) >= 0 && now - Date.parse(projection.observedAt) <= MAX_AGE_MS ? projection : null;
    })(),
  };
}
