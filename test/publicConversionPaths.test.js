...
test("the Tools hub lists only live builder utilities", async () => {
  ...
  assert.match(html, /<title>AI coding tools to reduce token usage \| 1stStep\.ai<\/title>/);
  assert.match(html, /Tools for your own build/);
  assert.match(html, /AI coding tools to/);
  assert.match(html, /reduce token usage/i);
  assert.match(html, /running out of usage|usage limit too fast|token burn/i);
  assert.match(html, /href="\/tools\/auto-model-router\/"/);
  assert.match(html, /<h2><a href="\/tools\/auto-model-router\/">Auto Model Router<\/a><\/h2>/);
  assert.match(html, /cheaper model for simple tasks|Don't send every task to Opus|instead of sending everything to Opus/i);
  assert.match(html, /does not shrink context/i);
  ...
  assert.doesNotMatch(html, /saves \d+%|always picks the right model/i);
  assert.doesNotMatch(html, /lean\.ctx|ponytail/i);
  assert.match(home, /<nav class="main-nav"[^>]*>[\s\S]*href="\/tools\/"/);
  assert.match(home, /<nav class="main-nav"[^>]*>[\s\S]*>Tools<\/a>/);
...
test("the Auto Model Router page is optional, honest, and opt-in", async () => {
  ...
  const sitemap = await source("public/sitemap.xml");
  const readme = await source("README.md");
  assert.match(html, /<title>Reduce Cursor and Claude token usage \| Auto Model Router<\/title>/);
  assert.match(html, /Use a cheaper/);
  assert.match(html, /confirm before it runs/i);
  assert.match(html, /does not shrink context by itself/i);
  assert.match(html, /href="\/tools\/"/);
  assert.match(html, /https:\/\/github.com\/1ststepai\/auto-model-router/);
  assert.match(html, /INSTALL\.md/);
  assert.match(html, /billing dashboard/i);
  assert.match(html, /mapped to a cheaper or faster model/i);
  assert.match(html, /relative estimates from a local/i);
  ...
  assert.match(html, /Does Auto Model Router automatically save tokens/);
  assert.match(html, /Will it always pick the right model/);
  assert.match(html, /lean\.ctx or ponytail/);
  assert.match(html, /new chat per task/i);
  assert.match(html, /\/compact/);
  assert.match(html, /Composer vs Opus/);
  assert.match(html, /example flow/i);
  assert.doesNotMatch(html, /saves \d+% automatically|always picks the right model/i);
