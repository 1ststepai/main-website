...
**Journey A roast/email candidate update (2026-09-14):** Evan requested email before the website/GitHub link and scan. The local candidate first asks for email and first-look consent, with a separate unchecked choice for occasional 1stStep.ai marketing updates. It then accepts a public link, requires an encrypted lead receipt before running the bounded public scan, and derives the free roast only from observed checks. It stores email, website origin or public GitHub repository, first-look consent, optional marketing choice/version, timestamp, and request ID for a proposed 90 days in the existing protected lead-store adapter; `/admin` Contact requests reads these OS records separately. It stores no scan result or private repository data, sends no report email, and does not add anyone to a marketing list or send campaigns. This is public-site contact intake, not G1 OS Project persistence or a full Free OS Audit. Hosted storage-provider/account and retention approval, exact-source Preview, admin readback, independent trust-boundary review, and Production authorization remain open gates.
