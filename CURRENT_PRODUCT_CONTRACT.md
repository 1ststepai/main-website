...
**Journey A roast/email candidate update (2026-09-14):** Evan requested a live scan followed by a free roast with email collection. The local candidate shows the public-source checks before a consented email form, then reveals a short response derived only from those checks after an encrypted lead receipt. It stores email, website origin or public GitHub repository, consent, timestamp, and request ID for a proposed 90 days in the existing protected lead-store adapter; it stores no scan result or private repository data, sends no report email, and does not subscribe the visitor to marketing. This is public-site contact intake, not G1 OS Project persistence or a full Free OS Audit. Hosted storage-provider/account and retention approval, exact-source Preview, admin readback, independent trust-boundary review, and Production authorization remain open gates.
