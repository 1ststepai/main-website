...
export function createFoundry(host, {tier="balanced", onSlow=()=>{}, started=performance.now()}={}){
 ...
 if(tier==='full') {
  ...
  env=pmrem.fromScene(environment,.04);scene.environment=env.texture;environment.dispose();pmrem.dispose();
  scene.add(new T.HemisphereLight(0xbce4ff,0x080a14,2));
  for(const [color,power,x,y,z]of [[0xc5eaff,100,2,6,6],[0x7b67ff,75,-5,0,3],[0x6dcaff,110,3,-3,2]]){const light=new T.PointLight(color,power);light.position.set(x,y,z);scene.add(light)}
  steel=new T.MeshStandardMaterial({color:0x425462,metalness:.82,roughness:.27});
  black=new T.MeshStandardMaterial({color:0x101b25,metalness:.65,roughness:.3});
 } else {
  // A tiny procedural matcap preserves beveled dimensional shading without lights/PMREM.
  const canvas=document.createElement('canvas');canvas.width=canvas.height=64;
  const ctx=canvas.getContext('2d'),gradient=ctx.createLinearGradient(0,0,50,64);
  gradient.addColorStop(0,'#ecfaff');gradient.addColorStop(.25,'#91b3c7');gradient.addColorStop(.5,'#3c5b72');gradient.addColorStop(.8,'#152638');gradient.addColorStop(1,'#6c88a5');ctx.fillStyle=gradient;ctx.fillRect(0,0,64,64);
  const matcap=new T.CanvasTexture(canvas);matcap.colorSpace=T.SRGBColorSpace;textures.push(matcap);
  steel=new T.MeshMatcapMaterial({matcap,color:0xb2c5d7});
  black=new T.MeshMatcapMaterial({matcap,color:0x263444});
 }
 ...
 const labelBatch=createLabels(scene,[...labels,'OWNER','SOURCE','INFRASTRUCTURE','ACCESS','HANDOFF DATA','HANDOFF DOCUMENTATION','HANDOFF EVIDENCE','HUMAN CONTROL']);
 textures.push(labelBatch.texture);
 function label(text,size=.2){return labelBatch.label(text,size)}
 function mesh(geo,mat,parent=root){const m=new T.Mesh(geo,mat);parent.add(m);return m}
 function ring(radius,tube,mat,parent=root){return mesh(new T.TorusGeometry(radius,tube,settings.radial,settings.segments),mat,parent)}
 function sector(inner,outer,start,length,depth){const s=new T.Shape();s.absarc(0,0,outer,start,start+length,false);s.absarc(0,0,inner,start+length,start,true);s.closePath();return new T.ExtrudeGeometry(s,{depth,bevelEnabled:settings.bevel>0,bevelSegments:settings.bevel,steps:1,bevelSize:.035,bevelThickness:.025,curveSegments:settings.curve})}
 ...
 for(let i=0;i<settings.teeth;i++){let a=i/settings.teeth*Math.PI*2;const tooth=mesh(new T.BoxGeometry(.025,.12,.07),i%4===0?signal:steel,core);tooth.position.set(Math.cos(a)*1.79,Math.sin(a)*1.79,.05);tooth.rotation.z=a-Math.PI/2}
 ...
 for(let j=0;j<settings.fins;j++){const ang=a+.085+j*.07;const fin=mesh(new T.BoxGeometry(.018,.3,.11),steel,g);fin.position.set(Math.cos(ang)*2.33,Math.sin(ang)*2.33,.3);fin.rotation.z=ang-Math.PI/2}
 const text=label(labels[i],.14);text.position.set(Math.cos(a+.24)*3,Math.sin(a+.24)*3,.3);g.add(text);modules.push({g,a});}
 const hoops=new T.Group();root.add(hoops);ring(3.18,.015,steel,hoops);if(tier!=='lite')ring(3.28,.006,signal,hoops);hoops.rotation.x=.24;
 ...
 const traces=[];for(let i=0;i<settings.traces;i++){const a=i*Math.PI*2/settings.traces+.24;const points=[new T.Vector3(Math.cos(a)*2.05,Math.sin(a)*2.05,.25),new T.Vector3(Math.cos(a)*1.92,Math.sin(a)*1.92,.62),new T.Vector3(Math.cos(a)*1.4,Math.sin(a)*1.4,.62)];const line=mesh(new T.TubeGeometry(new T.CatmullRomCurve3(points),settings.tube,.013,3,false),signal);traces.push(line)}
 ...
 resize();return{set(p,mode){target=p;path=mode;wake()},dispose(){meter.dispose();disposed=true;cancelAnimationFrame(raf);observer.disconnect();removeEventListener('resize',resize);removeEventListener('pointermove',pointer);document.removeEventListener('visibilitychange',visibility);scene.traverse(o=>{o.geometry?.dispose();if(o.material){for(const m of(Array.isArray(o.material)?o.material:[o.material]))m.dispose()}});textures.forEach(t=>t.dispose());env?.dispose();renderer.domElement.removeEventListener('webglcontextlost',lost);renderer.dispose();renderer.forceContextLoss();renderer.domElement.remove()}};
}
