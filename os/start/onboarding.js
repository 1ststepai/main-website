import { capabilityNames, deriveCapabilities, deriveGenome, flowForBusiness, getRecommendations, interpretGoal, modes, normalizeAuditTarget, questions, safeInspectedHref, suggestMode } from './onboarding-state.js';
...
const validHandoff = handedTarget && typeof firstLookHandoff?.receipt === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(firstLookHandoff.receipt) && typeof firstLookHandoff?.email === 'string' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(firstLookHandoff.email);
...
function renderAuditRequest() {
  ...
  return `${heading('02', 'PUBLIC FIRST LOOK', 'Your link. Real evidence.', 'We only report signals we can verify from a public response. This is not a full project, security, or release audit.')}
    <div class="interpretation-card"><span>YOUR ${label.toUpperCase()} / USER PROVIDED</span><strong class="audit-target-value"></strong></div>
...
async function runPublicScan() {
  ...
  try {
    const response = await fetch('/api/os-public-scan', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ request_id: roastRequest.receipt, url: target, source_type: state.auditTarget.sourceType }), signal: AbortSignal.timeout(12000) });
...
function render(focus = false) {
  ...
  const targetValue = conversation.querySelector('.audit-target-value');
  if (targetValue) targetValue.textContent = state.stage === 'audit-request' ? state.auditTarget?.url || '' : roastRequest.target?.url || '';
  const targetInput = conversation.querySelector('#audit-target-input');
  if (targetInput) targetInput.value = firstLookPrefill;
  const receiptValue = conversation.querySelector('#roast-receipt-value');
  if (receiptValue) receiptValue.textContent = roastRequest.receipt || '';
  const inspectedUrl = conversation.querySelector('#scan-inspected-url');
  if (inspectedUrl) {
    const value = publicScan.result?.inspectedUrl;
    inspectedUrl.textContent = typeof value === 'string' ? value : 'Source URL unavailable';
    const href = safeInspectedHref(value);
    if (href) {
      ...
      inspectedUrl.replaceChildren(link);
    }
  }
  const setupEmail = conversation.querySelector('#os-setup-email');
  if (setupEmail) setupEmail.value = roastRequest.email;
